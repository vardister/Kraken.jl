...
function run_pekeris(freq, params; type="full")
    ...
    ssp, layers, sspHS = pekeris_env(;
        c0 = cw, cb = cb, ρ0 = ρw, ρb = ρb, depth = depth
    )
...
function dkr_dparams_jl(params)
    dkr_dparams = ForwardDiff.jacobian(
        x -> run_pekeris(100.0, x; type="kr"),
        params
    )
    return dkr_dparams
end
...
function dkr_dfreq_jl(freq, params)
    dkr_dparams = ForwardDiff.derivative(
        x -> run_pekeris(x, params; type="kr"),
        freq
    )
    return dkr_dparams
end
...
function dkr_dparams_fortran(params)
    dkr_dparams = FiniteDiff.finite_difference_jacobian(
        x -> run_pekeris_fortran(100.0, x; type="kr"),
        params
    )
    return dkr_dparams
end
...
function dpsi_dparams_jl(params)
    dkr_dparams = ForwardDiff.jacobian(
        x -> run_pekeris(100.0, x; type="mode"),
        params
    )
    return dkr_dparams
end
...
function dpsi_dparams_fortran(params)
    dkr_dparams = FiniteDiff.finite_difference_jacobian(
        x -> run_pekeris_fortran(100.0, x; type="mode"),
        params
    ; absstep=1)
    return dkr_dparams
end