...
export AcousticProblemCache, bisection, solve_for_kr, inverse_iteration, det_sturm,
       kraken_jl, find_kr, get_g
...
"""
function det_sturm(kr, env::UnderwaterEnv, props::AcousticProblemProperties,cache::AcousticProblemCache; stop_at_k = nothing, scale = true)
  local p2, p1, p0, λ
  mode_count = 0
...
"""
function bisection(
    env::UnderwaterEnv, props::AcousticProblemProperties, cache::AcousticProblemCache)
  ω = 2pi * props.freq
...
"""
	find_kr(env::UnderwaterEnv, props::AcousticProblemProperties, cache::AcousticProblemCache; method=SimpleNewtonRaphson, kwargs...)

Find the roots of the acoustic problem.
"""
function find_kr(
    env::UnderwaterEnv, props::AcousticProblemProperties, cache::AcousticProblemCache;
    method = SimpleNewtonRaphson(), kwargs...
)
...
function solve_for_kr(span, env, props, cache; method = SimpleNewtonRaphson(), kwargs...)
  function f(u, p)
...
function inverse_iteration(kr, env::UnderwaterEnv, props::AcousticProblemProperties,
    cache::AcousticProblemCache; reltol = 0.01, dense = false)
...
function kraken_jl(env, freq; n_meshes = 5, rmax = 10_000, method = SimpleNewtonRaphson(),
    dont_break = false, abstol = 1e-6, reltol = 1e-6)
  # First mesh first
  local rich_krs
