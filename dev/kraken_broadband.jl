...
function pressure_field(
    mod_sol, r_vec::Vector, zs, zr_vec::Vector; t0_offset=0.1, min_modes=1, max_modes=typemax(Int), TL=false
)
    freq = mod_sol.props.freq
    max_modes = min(size(mod_sol.modes, 2), max_modes)
...
"""
function pressure_field_pekeris(env::PekerisUnderwaterEnv, freq, r, zs, zr; t0=0.1, n_points=2_000, max_modes=Inf)
    if freq == 0
        return 0.0 + 0.0im
    end
    ω = 2π * freq
    ...
    pf =
        ...
        sum([Ψ(zs, mode) * Ψ(zr, mode) * exp(-1im * kr[mode] * r) / sqrt(kr[mode]) for mode in range(1, n_modes)])
